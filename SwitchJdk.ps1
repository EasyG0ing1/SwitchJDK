...
$me = [System.Io.Path]::GetFileNameWithoutExtension($thisScriptFilePath)
...
if ($action -like '`?') {
    ...
    $message += "and it will create a folder inisde of your Documents folder called WindowsPowerShell (if it does not already exist) and will copy itself over to that folder and add that folder your environment Path variable.`n`n"
    ...
    $message += "`t$me 2`n`n"
    $message += "This will change the proper environment variables and will then run java -version so you can be sure that your environment is set up as desired.`n`n"
    $message += "The file that holds the list of JDK paths that you add is located here:`n`t$filePath`n`n"
    $message += "I hope this script is useful to you  :-)"
...
if ($action -like 'init') {
    ...
    $scriptPath = $userHome + "\Documents\WindowsPowerShell"
    $scriptPathExists = Test-Path -Path $scriptPath -PathType Container
    if (!$scriptPathExists) {
        New-Item -Path $scriptPath -ItemType Directory
    }
    $scriptFileName = Split-Path $thisScriptFilePath -Leaf
    $destinationScriptFilePath = $scriptPath + "\" + $scriptFileName
    Copy-Item $thisScriptFilePath $destinationScriptFilePath
    $envPathArry = $env:Path.Split(';')
    $pathExists = $false
    foreach($path in $envPathArry) {
        if($path -like $scriptPath) {
            $pathExists = $true
        }
    }
    if ($pathExists -eq $false) {
        $newEnvPath = $scriptPath + ";" + [Environment]::GetEnvironmentVariable("Path")
        [Environment]::SetEnvironmentVariable("Path",$newEnvPath, 'Machine')
        [Environment]::SetEnvironmentVariable("Path",$newEnvPath)
    }
    Write-Host "This script has been copied into the WindowsPowerShell folder inside your Documents folder and added to the Path environment variable.`nYou may delete the script from this location then simply use it from anywhere."
    exit
}
...
switch ($action) {
    ...
    default {
        ...
        [Environment]::SetEnvironmentVariable("JAVA_HOME", $jdkPathSwitch, 'Machine')
